...
async def create_slicing(payload: SlicingIn, _=Depends(require_permission(PERM_WRITE))):
    doc = payload.model_dump()
    doc["id"] = str(uuid.uuid4())
    # Timestamp completo (ISO con hora). Si el cliente pasa `fecha` (solo YYYY-MM-DD)
    # la guardamos como fecha lógica pero SIEMPRE registramos created_at con hora exacta
    # para tener trazabilidad temporal (requisito del cliente Feb-2026).
    doc["created_at"] = _now()
    doc["fecha"] = doc.get("fecha") or doc["created_at"][:10]
    doc["updated_at"] = _now()
    doc["source"] = "cms"
    doc["stock_consumed"] = None
    doc["salary_line_id"] = None
    ...
    # ENGRANAJE 4: generar línea de salario automáticamente según la tarifa del empleado
    emp = await db.production_employees.find_one({"id": payload.empleado_id})
    if emp and (payload.peso_loncheado or 0) > 0:
        tarifa = float(
            emp.get("tarifa_loncheado_emplatado") if payload.emplatado or payload.tipo == "EMPLATADO"
            else emp.get("tarifa_loncheado_normal") or 0
        )
        if tarifa > 0:
            importe = round(float(payload.peso_loncheado) * tarifa, 2)
            salary = {
                "id": str(uuid.uuid4()),
                "empleado_id": payload.empleado_id,
                "tipo": "EMPLATADO" if (payload.emplatado or payload.tipo == "EMPLATADO") else "NORMAL",
                "peso_loncheado": float(payload.peso_loncheado),
                "tarifa_kg": tarifa,
                "importe": importe,
                "loncheado_id": doc["id"],
                "fecha": doc["fecha"],
            ...
            await db.production_salaries.insert_one(salary)
            doc["salary_line_id"] = salary["id"]
            doc["salary_importe"] = importe
...
async def delete_slicing(sid: str, _=Depends(require_permission(PERM_WRITE))):
    ...
    if not existing:
        raise HTTPException(404, "Loncheado no encontrado")
    # Rollback salary line generated by this slicing (engranaje inverso)
    await db.production_salaries.delete_many({"loncheado_id": sid, "source": "auto:slicing"})
    await db.production_slicings.delete_one({"id": sid})
    return {"ok": True}


# ---------------------------------------------------------- Analítica producción

@router.get("/analytics/evolution")
async def slicings_evolution(
    days: int = Query(30, ge=1, le=365),
    _=Depends(require_permission(PERM_READ)),
):
    """Serie diaria de kilos loncheados + ingresos + coste, últimos N días."""
    ...
    for i in range(days):
        ...
        days_map[d] = {"day": d, "kg": 0.0, "ingresos": 0.0, "coste": 0.0, "count": 0}
    async for s in db.production_slicings.find(
        {"fecha": {"$gte": since}},
        {"_id": 0, "fecha": 1, "peso_loncheado": 1, "precio_cliente": 1, "coste": 1},
    ):
        d = (s.get("fecha") or "")[:10]
        if d in days_map:
            days_map[d]["kg"] += float(s.get("peso_loncheado") or 0)
            days_map[d]["ingresos"] += float(s.get("precio_cliente") or 0)
            days_map[d]["coste"] += float(s.get("coste") or 0)
            days_map[d]["count"] += 1
    rows = [{**v, "kg": round(v["kg"], 2), "ingresos": round(v["ingresos"], 2),
             "coste": round(v["coste"], 2)} for v in days_map.values()]
    return {"days": days, "rows": rows}


@router.get("/analytics/top-employees")
async def top_employees(
    ...
    out = []
    async for r in db.production_slicings.aggregate(pipe):
        emp = employees.get(r["_id"], {})
        out.append({
            "empleado_id": r["_id"],
            "nombre": emp.get("nombre", "—"),
            "kg": round(float(r.get("kg", 0)), 2),
            "ingresos": round(float(r.get("ingresos", 0)), 2),
            "piezas": int(r["piezas"]),
        })
...
async def top_clients(
    ...
    clients = {c["id"]: c async for c in db.production_clients.find({}, {"_id": 0})}
    out = []
    async for r in db.production_slicings.aggregate(pipe):
        cli = clients.get(r["_id"], {})
        out.append({
            "cliente_id": r["_id"],
...
async def top_products(
    ...
    products = {p["id"]: p async for p in db.production_products.find({}, {"_id": 0})}
    out = []
    async for r in db.production_slicings.aggregate(pipe):
