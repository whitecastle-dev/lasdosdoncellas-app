...
export default function StoreHeader({ onOpenCart }) {
  ...
  // Llamada a los hooks siempre en el mismo orden, sin try/catch
  ...
  // Acceso seguro a las propiedades del contexto
    ...
    >
      <div className="max-w-[1500px] mx-auto px-6 lg:px-12 flex items-center justify-between py-4">
        ...
        <nav className="hidden md:flex items-center gap-10" style={{ color: "#FAF8F5" }}>
          <Link to="/" className="nav-link" data-testid="nav-home">{t("Inicio")}</Link>
          <Link to="/catalogo" className="nav-link" data-testid="nav-catalogo">{t("Catálogo")}</Link>
          <Link to="/lotes/configurador" className="nav-link" data-testid="nav-configurador">{t("Configurar lote")}</Link>
          <Link to="/nosotros" className="nav-link" data-testid="nav-nosotros">{t("Nosotros")}</Link>
        </nav>
        <div className="flex items-center gap-4">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-[#0a0a0a] border border-[rgba(197,160,89,0.3)] py-2 shadow-xl z-50">
                  <Link to="/cuenta/perfil" className="block px-4 py-2 text-xs text-[#FAF8F5] hover:bg-[#C5A059] hover:text-black transition uppercase tracking-widest" onClick={() => setIsDropdownOpen(false)}>{t("Mi perfil")}</Link>
                  <Link to="/cuenta?tab=orders" className="block px-4 py-2 text-xs text-[#FAF8F5] hover:bg-[#C5A059] hover:text-black transition uppercase tracking-widest" onClick={() => setIsDropdownOpen(false)}>{t("Mis pedidos")}</Link>
                  <Link to="/cuenta/configuracion" className="block px-4 py-2 text-xs text-[#FAF8F5] hover:bg-[#C5A059] hover:text-black transition uppercase tracking-widest" onClick={() => setIsDropdownOpen(false)}>{t("Cambiar contraseña")}</Link>
                  <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-xs text-red-400 hover:bg-red-900/20 transition uppercase tracking-widest">{t("Cerrar sesión")}</button>
                </div>
              )}
            </div>
          ) : (
          ...
          <button onClick={onOpenCart} data-testid="header-cart-button" className="relative px-3 py-2 text-[#FAF8F5] hover:text-[#C5A059] transition flex items-center gap-2">
            <ShoppingBag size={18} />
            <span className="hidden sm:inline label-eyebrow">{t("Cesta")}</span>
            {count > 0 && (
      ...
      {open && (
        <div className="md:hidden border-t border-[rgba(197,160,89,0.18)] px-6 py-4 flex flex-col gap-3" style={{ background: "rgba(10,10,10,0.95)", color: "#FAF8F5" }}>
          <Link to="/" className="nav-link" onClick={() => setOpen(false)}>{t("Inicio")}</Link>
          <Link to="/catalogo" className="nav-link" onClick={() => setOpen(false)}>{t("Catálogo")}</Link>
          <Link to="/lotes/configurador" className="nav-link" onClick={() => setOpen(false)}>{t("Configurar lote")}</Link>
          <Link to="/nosotros" className="nav-link" onClick={() => setOpen(false)}>{t("Nosotros")}</Link>
          <Link to={customer ? "/cuenta" : "/cuenta/login"} className="nav-link" onClick={() => setOpen(false)}>
            {customer ? t("Mi cuenta") : t("Acceder")}
          </Link>
        </div>
      )}
    </header>
